...
typedef struct GLGE_MAT_STRUCT_NAME {
     ...
     */
    GLGE_MAT_VEC_TYPE m[4]
    //check for C++ to define a default
    #if GLGE_CPP
     = {GLGE_MAT_VEC_TYPE(1,0,0,0), GLGE_MAT_VEC_TYPE(0,1,0,0), GLGE_MAT_VEC_TYPE(0,0,1,0), GLGE_MAT_VEC_TYPE(0,0,0,1)}
     ...
     * @param m10 the upper-middle left element of the matrix
     * @param m11 the upper-middle middle-left element of the matrix
     * @param m12 the upper-middle middle-right element of the matrix
     * @param m13 the upper-middle right element of the matrix
     * @param m20 the lower-middle left element of the matrix
     * @param m21 the lower-middle middle-left element of the matrix
     * @param m22 the lower-middle middle-right element of the matrix
     * @param m23 the lower-middle right element of the matrix
     ...
     */
    GLGE_MAT_STRUCT_NAME(GLGE_MAT_TYPE m00, GLGE_MAT_TYPE m01, GLGE_MAT_TYPE m02, GLGE_MAT_TYPE m03, 
                         GLGE_MAT_TYPE m10, GLGE_MAT_TYPE m11, GLGE_MAT_TYPE m12, GLGE_MAT_TYPE m13, 
                         GLGE_MAT_TYPE m20, GLGE_MAT_TYPE m21, GLGE_MAT_TYPE m22, GLGE_MAT_TYPE m23,
                         GLGE_MAT_TYPE m30, GLGE_MAT_TYPE m31, GLGE_MAT_TYPE m32, GLGE_MAT_TYPE m33)
    {
        //store the elements as vectors
        m[0] = GLGE_MAT_VEC_TYPE(m00, m01, m02, m03);
        m[1] = GLGE_MAT_VEC_TYPE(m10, m11, m12, m13);
        m[2] = GLGE_MAT_VEC_TYPE(m20, m21, m22, m23);
        m[3] = GLGE_MAT_VEC_TYPE(m30, m31, m32, m33);
    }
     ...
     * @param row1 the uppder-middle row of the matrix
     * @param row2 the lower-middle row of the matrix
     * @param row3 the lower row of the matrix
     */
    GLGE_MAT_STRUCT_NAME(GLGE_MAT_VEC_TYPE row0, GLGE_MAT_VEC_TYPE row1, GLGE_MAT_VEC_TYPE row2, GLGE_MAT_VEC_TYPE row3)
    {
        //store all the rows of the matrix
        m[0] = row0;
        m[1] = row1;
        m[2] = row2;
        m[3] = row3;
    ...
    GLGE_MAT_STRUCT_NAME(GLGE_MAT_TYPE* array)
    {
        ...
        m[0] = GLGE_MAT_VEC_TYPE(array[0], array[1], array[2], array[3]);
        m[1] = GLGE_MAT_VEC_TYPE(array[4], array[5], array[6], array[7]);
        m[2] = GLGE_MAT_VEC_TYPE(array[8], array[9], array[10], array[11]);
        m[3] = GLGE_MAT_VEC_TYPE(array[12], array[13], array[14], array[15]);
    }
    ...
    inline GLGE_MAT_STRUCT_NAME operator+(const GLGE_MAT_STRUCT_NAME& other) const noexcept
    {
        ...
        return GLGE_MAT_STRUCT_NAME(
            m[0].x + other.m[0].x, m[0].y + other.m[0].y, m[0].z + other.m[0].z, m[0].w + other.m[0].w,
            m[1].x + other.m[1].x, m[1].y + other.m[1].y, m[1].z + other.m[1].z, m[1].w + other.m[1].w,
            m[2].x + other.m[2].x, m[2].y + other.m[2].y, m[2].z + other.m[2].z, m[2].w + other.m[2].w,
            m[3].x + other.m[3].x, m[3].y + other.m[3].y, m[3].z + other.m[3].z, m[3].w + other.m[3].w
        );
    }
    ...
    inline GLGE_MAT_STRUCT_NAME operator+(GLGE_MAT_TYPE scalar) const noexcept
    {
        ...
        return GLGE_MAT_STRUCT_NAME(
            m[0].x + scalar, m[0].y + scalar, m[0].z + scalar, m[0].w + scalar,
            m[1].x + scalar, m[1].y + scalar, m[1].z + scalar, m[1].w + scalar,
            m[2].x + scalar, m[2].y + scalar, m[2].z + scalar, m[2].w + scalar,
            m[3].x + scalar, m[3].y + scalar, m[3].z + scalar, m[3].w + scalar
        );
    }
    ...
    inline GLGE_MAT_STRUCT_NAME operator*(const GLGE_MAT_STRUCT_NAME& other) const noexcept
    {
        ...
        return GLGE_MAT_STRUCT_NAME(
            m[0].x * other.m[0].x + m[0].y * other.m[1].x + m[0].z * other.m[2].x + m[0].w * other.m[3].x, m[0].x * other.m[0].y + m[0].y * other.m[1].y + m[0].z * other.m[2].y + m[0].w * other.m[3].y, m[0].x * other.m[0].z + m[0].y * other.m[1].z + m[0].z * other.m[2].z + m[0].w * other.m[3].z, m[0].x * other.m[0].w + m[0].y * other.m[1].w + m[0].z * other.m[2].w + m[0].w * other.m[3].w, 
            m[1].x * other.m[0].x + m[1].y * other.m[1].x + m[1].z * other.m[2].x + m[1].w * other.m[3].x, m[1].x * other.m[0].y + m[1].y * other.m[1].y + m[1].z * other.m[2].y + m[1].w * other.m[3].y, m[1].x * other.m[0].z + m[1].y * other.m[1].z + m[1].z * other.m[2].z + m[1].w * other.m[3].z, m[1].x * other.m[0].w + m[1].y * other.m[1].w + m[1].z * other.m[2].w + m[1].w * other.m[3].w, 
            m[2].x * other.m[0].x + m[2].y * other.m[1].x + m[2].z * other.m[2].x + m[2].w * other.m[3].x, m[2].x * other.m[0].y + m[2].y * other.m[1].y + m[2].z * other.m[2].y + m[2].w * other.m[3].y, m[2].x * other.m[0].z + m[2].y * other.m[1].z + m[2].z * other.m[2].z + m[2].w * other.m[3].z, m[2].x * other.m[0].w + m[2].y * other.m[1].w + m[2].z * other.m[2].w + m[2].w * other.m[3].w, 
            m[3].x * other.m[0].x + m[3].y * other.m[1].x + m[3].z * other.m[2].x + m[3].w * other.m[3].x, m[3].x * other.m[0].y + m[3].y * other.m[1].y + m[3].z * other.m[2].y + m[3].w * other.m[3].y, m[3].x * other.m[0].z + m[3].y * other.m[1].z + m[3].z * other.m[2].z + m[3].w * other.m[3].z, m[3].x * other.m[0].w + m[3].y * other.m[1].w + m[3].z * other.m[2].w + m[3].w * other.m[3].w 
        );
    }
    ...
    inline GLGE_MAT_VEC_TYPE operator*(const GLGE_MAT_VEC_TYPE& vector) const noexcept
    {
        ...
        return GLGE_MAT_VEC_TYPE(
            vector.x * m[0].x + vector.y * m[0].y + vector.z * m[0].z + vector.w * m[0].w,
            vector.x * m[1].x + vector.y * m[1].y + vector.z * m[1].z + vector.w * m[1].w,
            vector.x * m[2].x + vector.y * m[2].y + vector.z * m[2].z + vector.w * m[2].w,
            vector.x * m[3].x + vector.y * m[3].y + vector.z * m[3].z + vector.w * m[3].w
        );
    }
    ...
    inline double determinant() const noexcept {
        ...
        return (
            //m[0].x * det(M00)
            m[0].x * (m[1].y * (m[2].z * m[3].w - m[2].w * m[3].z) - m[1].z * (m[2].y * m[3].w - m[2].w * m[3].y) + m[1].w * (m[2].y * m[3].z - m[2].z * m[3].y)) - 
            //m[0].y * det(M01)
            m[0].y * (m[1].x * (m[2].z * m[3].w - m[2].w * m[3].z) - m[1].z * (m[2].x * m[3].w - m[2].w * m[3].x) + m[1].w * (m[2].x * m[3].z - m[2].z * m[3].x)) + 
            //m[0].z * det(M02)
            m[0].z * (m[1].x * (m[2].y * m[3].w - m[2].w * m[3].y) - m[1].y * (m[2].x * m[3].w - m[2].w * m[3].x) + m[1].w * (m[2].x * m[3].y - m[2].y * m[3].x)) - 
            //m[0].w * det(M03)
            m[0].w * (m[1].x * (m[2].y * m[3].z - m[2].z * m[3].y) - m[1].y * (m[2].x * m[3].z - m[2].z * m[3].x) + m[1].z * (m[2].x * m[3].y - m[2].y * m[3].x))
        );
    }
    ...
    /**
     * @brief compute the cofactor matrix of the 4x4 matrix
     * 
    ...
    inline GLGE_MAT_STRUCT_NAME cofactors() const noexcept
    {
        ...
        return GLGE_MAT_STRUCT_NAME(
            //row 0
              (m[1].y * m[2].z * m[3].w + m[1].z * m[2].w * m[3].y + m[1].w * m[2].y * m[3].z
               -m[1].w * m[2].z * m[3].y - m[1].z * m[2].y * m[3].w - m[1].y * m[2].w * m[3].z),
             -(m[1].x * m[2].z * m[3].w + m[1].z * m[2].w * m[3].x + m[1].w * m[2].x * m[3].z
               -m[1].w * m[2].z * m[3].x - m[1].z * m[2].x * m[3].w - m[1].x * m[2].w * m[3].z),
              (m[1].x * m[2].y * m[3].w + m[1].y * m[2].w * m[3].x + m[1].w * m[2].x * m[3].y
               -m[1].w * m[2].y * m[3].x - m[1].y * m[2].x * m[3].w - m[1].x * m[2].w * m[3].y),
             -(m[1].x * m[2].y * m[3].z + m[1].y * m[2].z * m[3].x + m[1].z * m[2].x * m[3].y
               -m[1].z * m[2].y * m[3].x - m[1].y * m[2].x * m[3].z - m[1].x * m[2].z * m[3].y),
            //row 1
             -(m[0].y * m[2].z * m[3].w + m[0].z * m[2].w * m[3].y + m[0].w * m[2].y * m[3].z
               -m[0].w * m[2].z * m[3].y - m[0].z * m[2].y * m[3].w - m[0].y * m[2].w * m[3].z),
              (m[0].x * m[2].z * m[3].w + m[0].z * m[2].w * m[3].x + m[0].w * m[2].x * m[3].z
               -m[0].w * m[2].z * m[3].x - m[0].z * m[2].x * m[3].w - m[0].x * m[2].w * m[3].z),
             -(m[0].x * m[2].y * m[3].w + m[0].y * m[2].w * m[3].x + m[0].w * m[2].x * m[3].y
               -m[0].w * m[2].y * m[3].x - m[0].y * m[2].x * m[3].w - m[0].x * m[2].w * m[3].y),
              (m[0].x * m[2].y * m[3].z + m[0].y * m[2].z * m[3].x + m[0].z * m[2].x * m[3].y
               -m[0].z * m[2].y * m[3].x - m[0].y * m[2].x * m[3].z - m[0].x * m[2].z * m[3].y),
            //row 2
              (m[0].y * m[1].z * m[3].w + m[0].z * m[1].w * m[3].y + m[0].w * m[1].y * m[3].z
               -m[0].w * m[1].z * m[3].y - m[0].z * m[1].y * m[3].w - m[0].y * m[1].w * m[3].z),
             -(m[0].x * m[1].z * m[3].w + m[0].z * m[1].w * m[3].x + m[0].w * m[1].x * m[3].z
               -m[0].w * m[1].z * m[3].x - m[0].z * m[1].x * m[3].w - m[0].x * m[1].w * m[3].z),
              (m[0].x * m[1].y * m[3].w + m[0].y * m[1].w * m[3].x + m[0].w * m[1].x * m[3].y
               -m[0].w * m[1].y * m[3].x - m[0].y * m[1].x * m[3].w - m[0].x * m[1].w * m[3].y),
             -(m[0].x * m[1].y * m[3].z + m[0].y * m[1].z * m[3].x + m[0].z * m[1].x * m[3].y
               -m[0].z * m[1].y * m[3].x - m[0].y * m[1].x * m[3].z - m[0].x * m[1].z * m[3].y),
            //row 3
             -(m[0].y * m[1].z * m[2].w + m[0].z * m[1].w * m[2].y + m[0].w * m[1].y * m[2].z
               -m[0].w * m[1].z * m[2].y - m[0].z * m[1].y * m[2].w - m[0].y * m[1].w * m[2].z),
              (m[0].x * m[1].z * m[2].w + m[0].z * m[1].w * m[2].x + m[0].w * m[1].x * m[2].z
               -m[0].w * m[1].z * m[2].x - m[0].z * m[1].x * m[2].w - m[0].x * m[1].w * m[2].z),
             -(m[0].x * m[1].y * m[2].w + m[0].y * m[1].w * m[2].x + m[0].w * m[1].x * m[2].y
               -m[0].w * m[1].y * m[2].x - m[0].y * m[1].x * m[2].w - m[0].x * m[1].w * m[2].y),
              (m[0].x * m[1].y * m[2].z + m[0].y * m[1].z * m[2].x + m[0].z * m[1].x * m[2].y
               -m[0].z * m[1].y * m[2].x - m[0].y * m[1].x * m[2].z - m[0].x * m[1].z * m[2].y)
        );
    }
...
{return os << GLGE_STRINGIFY_DEFINE(GLGE_MAT_NAME) << "{{" << mat.m[0].x << ", " << mat.m[0].y << ", " << mat.m[0].z << ", " << mat.m[0].w << "}, {" 
