...
public:
    ...
    bool open(const std::filesystem::path& path) {
        ...
        m_handle = LoadLibraryExW(p.c_str(), nullptr, LOAD_LIBRARY_SEARCH_DLL_LOAD_DIR | LOAD_LIBRARY_SEARCH_DEFAULT_DIRS);
        ...
        m_handle = dlopen(p.c_str(), RTLD_NOW | RTLD_LOCAL);
        //handle remains `nullptr` on failure
        return m_handle != nullptr;
    #endif
    }
     ...
     * @return `std::pair<T, bool>` the loaded function in the first element, the second element stores `true` if the load was successful and `false` if the load failed
     */
    template <typename T>
    requires (std::is_pointer_v<T> && std::is_function_v<std::remove_pointer_t<T>>)
    std::pair<T, bool> symbol(const char* name) const {
        //sanity check that the handle is not null
        if (m_handle == nullptr)
        {return {nullptr, false};}
        //get the address and reinterpret-cast it to provided type
    #if defined(_WIN32)
        auto* ptr = reinterpret_cast<T>(GetProcAddress(m_handle, name));
        //return the pointer and check if something went wrong
        return {ptr, ptr != nullptr};
    #else
        auto* ptr = reinterpret_cast<T>(dlsym(m_handle, name));
        //check for errors
